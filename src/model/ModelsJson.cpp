...
PrintedPage fromPrintedPageJson(const QString& json) {
    ...
    if (parse_error.error != QJsonParseError::NoError) {
        throw std::runtime_error("Parse error:" + parse_error.errorString().toStdString());
    }
    PrintedPage page;
    if (!doc.isNull() && doc.isObject()) {
        ...
        if (obj["id"].isDouble()) {
            ...
            throw std::runtime_error("Parse error: id is not a number");
        }

        if (obj["config_id"].isDouble()) {
            page.config_id = obj["config_id"].toInt();
        } else {
            throw std::runtime_error("Parse error: config_id is not a number");
        }

        if (obj["time"].isString()) {
            page.time = obj["time"].toString();
        } else {
            throw std::runtime_error("Parse error: time is not a string");
        }

        if (obj["status"].isString()) {
            page.status = obj["status"].toString();
        } else {
            throw std::runtime_error("Parse error: status is not a string");
        }

        if (obj["from_ip"].isString()) {
            page.from_ip = obj["from_ip"].toString();
        } else {
            throw std::runtime_error("Parse error: from_ip is not a string");
        }

        if (obj["page_url"].isString()) {
            page.page_url = obj["page_url"].toString();
        } else {
            throw std::runtime_error("Parse error: page_url is not a string");
        }

        if (obj["end_cmd_exec_message"].isString()) {
            page.end_cmd_exec_message = obj["end_cmd_exec_message"].toString();
        } else {
            throw std::runtime_error("Parse error: end_cmd_exec_message is not a string");
        }
    }
    return page;
}
